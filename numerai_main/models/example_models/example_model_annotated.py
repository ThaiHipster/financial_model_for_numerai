...
# Call to the Numerai API and assign it to "napi"
napi = NumerAPI()

spinner = Halo(text='', spinner='dots')

# Calling the current Numerai Tournament
current_round = napi.get_current_round(tournament=8)  # tournament 8 is the primary Numerai Tournament

# Tournament data changes every week so we specify the round in their name. Training
# and validation data only change periodically, so no need to download them every time.

# Label for when the dataset download begins
print('Downloading dataset files...')

# Using the "download_dataset" fucntion from the Numerai API to get the needed data
napi.download_dataset("numerai_training_data.parquet", "training_data.parquet")
napi.download_dataset("numerai_tournament_data.parquet", f"tournament_data_{current_round}.parquet") #Getting current round data which is the main new data
...
# Label for displaying the metadata
...
# 3. Modeling Section of the code
model_name = f"model_target"  #name of the checked for model, currently this exists but if name is changed it won't be found

# Printing the title of the model choosing selection
print(f"Checking for existing model '{model_name}'")
model = load_model(model_name) #attempting to load the named model

# Here a new model is built if the given name of the model is not found
if not model:
    ...
    spinner.start('Training model') #starting a spinner which spins until the model is trained
...
# 4.Checking the model and making predictions
